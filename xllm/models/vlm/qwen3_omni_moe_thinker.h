...
public:
    ...
    torch::Tensor forward(
        ...
        auto [feat_lengths_cnn ,feature_lens_after_cnn] = get_feat_extract_output_lengths(chunk_lengths);
        ...
        std::vector<int64_t> cu_chunk_lens = {0};
        for (int64_t i = 0; i < aftercnn_lens_calc.size(0); ++i) {
            int64_t cnn_len = aftercnn_lens_calc[i].item<int64_t>();
...
REGISTER_CAUSAL_VLM_MODEL(qwen3_omni_moe, Qwen3_Omni_Moe_Thinker_ForConditionalGeneration);
