...
REGISTER_INPUT_PROCESSOR(qwen3_omni_moe, Qwen2_5_VLInputProcessor);
...
REGISTER_MODEL_ARGS(qwen3_omni_moe, [&] {
  ...
  *******
  ...
  LOAD_ARG_WITH_PREFIX_JSON("talker_config.text_config", [&]{
      ...
      LOAD_ARG_OR_PREFIX(max_position_embeddings,
  "max_position_embeddings",65536); LOAD_ARG_OR_PREFIX(rope_theta,
  "rope_theta",1000000);
      // Attention
      ...
      // resize MLP
      LOAD_ARG_OR_PREFIX(talker_text_intermediate_size, "intermediate_size",
  2048); LOAD_ARG_OR_PREFIX(talker_text_hidden_size, "hidden_size", 1024);
   });
  ...
  */
  ...
  LOAD_ARG_WITH_PREFIX_JSON("code2wav_config", [&] {
    LOAD_ARG_OR_PREFIX(code2wav_config_codebook_size, "codebook_size", 2048);
    LOAD_ARG_OR_PREFIX(code2wav_config_hidden_size, "hidden_size", 1024);
    LOAD_ARG_OR_PREFIX(code2wav_config_max_position_embeddings,
                       "max_position_embeddings",
                       8000);
    LOAD_ARG_OR_PREFIX(code2wav_config_rope_theta, "rope_theta", 10000.0);
    LOAD_ARG_OR_PREFIX(
        code2wav_config_num_attention_heads, "num_attention_heads", 16);
    LOAD_ARG_OR_PREFIX(
        code2wav_config_num_key_value_heads, "num_key_value_heads", 16);
    LOAD_ARG_OR_PREFIX(code2wav_config_attention_bias, "attention_bias", false);
    LOAD_ARG_OR_PREFIX(code2wav_config_sliding_window, "sliding_window", 72);
    LOAD_ARG_OR_PREFIX(
        code2wav_config_intermediate_size, "intermediate_size", 3072);
    LOAD_ARG_OR_PREFIX(
        code2wav_config_hidden_act, "hidden_act", std::string("silu"));
    LOAD_ARG_OR_PREFIX(code2wav_config_layer_scale_initial_scale,
                       "layer_scale_initial_scale",
                       0.01);
    LOAD_ARG_OR_PREFIX(code2wav_config_rms_norm_eps, "rms_norm_eps", 1e-5);
    LOAD_ARG_OR_PREFIX(
        code2wav_config_num_hidden_layers, "num_hidden_layers", 8);
    LOAD_ARG_OR_PREFIX(code2wav_config_num_quantizers, "num_quantizers", 16);
    LOAD_ARG_OR_PREFIX(code2wav_config_decoder_dim, "decoder_dim", 1536);
    LOAD_ARG_OR_PREFIX(
        code2wav_config_attention_dropout, "attention_dropout", 0.0);
    LOAD_ARG_OR_PREFIX(code2wav_config_upsampling_ratios_vec,
                       "upsampling_ratios_vec",
                       (std::vector<int>{2, 2}));
    LOAD_ARG_OR_PREFIX(code2wav_config_upsample_rates_vec,
                       "upsample_rates_vec",
                       (std::vector<int>{8, 5, 4, 3}));
  });
});
}  // namespace xllm
