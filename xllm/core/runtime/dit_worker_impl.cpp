...
folly::SemiFuture<bool> DiTWorkerImpl::init_model_async(
  ...
  threadpool_.schedule(
      [this, model_weights_path, random_seed, promise]() mutable {
        bool status = this->init_model(model_weights_path, random_seed);
...
void DiTWorkerImpl::process_group_test() {
  ...
  parallel_state::reduce(tensor,
                         dit_context_.get_parallel_args().process_group_);
  // call allgather
