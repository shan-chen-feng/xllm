...
#endif
    hot_token_id_ = draft_impl_->get_hot_token_id();
  }
  return result;
}
...
SampleOutput SpeculativeWorkerImpl::validate(
  ...
  const int32_t target_vocab_size = target_output.logits.size(/*dim=*/-1);
  const int32_t draft_vocab_size =
      draft_outputs[0].sample_output.probs.size(/*dim=*/-1);
  ...
  // [batch_size, n_speculative_tokens, target_vocab_size]
  auto target_logits = target_output.logits.view(
      {batch_size, num_val_tokens, target_vocab_size});
  ...
  for (const auto& draft_output : draft_outputs) {
    ...
    auto draft_probs = draft_output.sample_output.probs.view(
        {{batch_size, 1, draft_vocab_size}});
