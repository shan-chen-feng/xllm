...
void ResidualCache::init(const DiTCacheConfig& cfg) {
  ...
  CHECK_GE(cfg.residual_cache.dit_cache_end_steps, 0)
      << "dit_cache_stop_steps must be >= 0";
  ...
  CHECK_GE(cfg.residual_cache.dit_cache_end_blocks, 0)
      << "dit_cache_stop_blocks must be >= 0";
  ...
  LOG(INFO) << dit_cache_start_steps_;
  LOG(INFO) << dit_cache_end_steps_;
  LOG(INFO) << dit_cache_start_blocks_;
  LOG(INFO) << dit_cache_end_blocks_;
  reset_cache();
}
