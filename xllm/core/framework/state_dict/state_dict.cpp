...
StateDict StateDict::get_dict_with_renamed_prefix(const std::string& old_prefix, 
    ...
    if ((old_end_with_dot && !new_end_with_dot) || (!old_end_with_dot && new_end_with_dot)) {
        LOG(WARNING)
          << "The input does not comply the standards"
          << "you are changing statedict prefixs from "
