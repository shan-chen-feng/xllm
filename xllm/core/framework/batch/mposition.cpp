...
torch::Tensor get_llm_pos_ids_for_vision(
    int start_idx,
    int vision_idx,
  ...
  if (seq_len > 0 && (image_grid_thw.defined() || video_grid_thw.defined())){ 
    ...
    if (video_grid_thw.defined()) {
        ...
        second_per_grids = torch::ones(
            {video_grid_thw.size(0)}, 
            torch::TensorOptions().dtype(torch::kFloat32)
        ) * second_per_grid_ts_value;
    }
    auto vision_start_mask = input_ids == vision_start_token_id;
    torch::Tensor vision_start_indices = torch::nonzero(vision_start_mask).squeeze(1);
    ...
    if (vision_start_indices.defined()) {
        ...
        vision_tokens = torch::empty({0}, torch::TensorOptions().dtype(torch::kLong));
    }
    ...
    for (int i = 0; i < multimodal_nums; ++i) {
        int st_idx = llm_pos_ids_list.empty() ? 0 : 
        ...
        LOG(INFO) << 2;
        if (audio_token_id == *std::find(input_tokens_vec.begin() + st, input_tokens_vec.end(), audio_token_id) && 
            remain_audios > 0) {
            auto it = std::find(input_tokens_vec.begin() + st, input_tokens_vec.end(), 
        ...
        if (min_ed == ed_audio_start) {
            auto audio_len = get_feat_extract_output_lengths(audio_seqlens[audio_idx]);
            ...
            auto t_index = (torch::arange(grid_t, torch::kInt32) * 
                          ...
                          position_id_per_seconds).to(torch::kInt32);
            LOG(INFO) << 8;
            ...
            while (video_idx_pos < video_pos_ids.size(1) && audio_idx_pos < audio_pos_ids.size(1)) {
        ...
        LOG(INFO) << 13;
        st_idx = llm_pos_ids_list.empty() ? 0 :
            llm_pos_ids_list.back().max().item<int>() + 1;
    ...
    LOG(INFO) << 17;
    int mrope_position_delta = llm_positions.max().item<int>() + 1 - input_tokens_vec.size();
    LOG(INFO) << 18;
