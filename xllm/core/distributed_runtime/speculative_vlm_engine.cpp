...
SpeculativeVLMEngine::SpeculativeVLMEngine(const runtime::Options& options)
    : options_(options) {
  ...
  engine_ = std::make_unique<VLMEngine>(engine_options, dist_manager_);

  const std::string draft_model_path = options_.draft_model_path().value_or("");
  CHECK(!draft_model_path.empty())
      << "draft model path is required for SpeculativeVLMEngine";

  // draft engine
  engine_options.model_path(draft_model_path)
...
bool SpeculativeVLMEngine::init() {
  if (!init_model()) {
    return false;
  }
...
bool SpeculativeVLMEngine::init_model() {
...
bool SpeculativeVLMEngine::allocate_kv_cache() {
...
ForwardOutput SpeculativeVLMEngine::step(std::vector<Batch>& batches) {
  return engine_->step(batches);
}

int64_t SpeculativeVLMEngine::calculate_kv_cache(
...
void SpeculativeVLMEngine::update_last_step_result(std::vector<Batch>& batch) {
  engine_->update_last_step_result(batch);
}

std::vector<int64_t> SpeculativeVLMEngine::get_active_activation_memory()
    const {
  return engine_->get_active_activation_memory();
}

bool SpeculativeVLMEngine::pull_kv_blocks(
...
void SpeculativeVLMEngine::get_device_info(std::vector<std::string>& device_ips,
                                           std::vector<uint16_t>& ports) {
  engine_->get_device_info(device_ips, ports);
}

void SpeculativeVLMEngine::get_cache_info(std::vector<uint64_t>& cluster_ids,
...
bool SpeculativeVLMEngine::link_cluster(
...
bool SpeculativeVLMEngine::unlink_cluster(
    const std::vector<uint64_t>& cluster_ids,
    const std::vector<std::string>& addrs,
    const std::vector<std::string>& device_ips,
    const std::vector<uint16_t>& ports,
