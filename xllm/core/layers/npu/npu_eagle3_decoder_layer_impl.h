...
class NpuEagle3DecoderLayerImpl : public BaseLayer {
 ...
 private:
  ...
  // Two decode nodes, mirroring the qwen3 decoder layer: the graph node builds
  // paged attention as the ACL-graph host-planned-tiling variant (consumes
  // paged_attention_tiling_data), the eager node uses dynamic per-step tiling.
