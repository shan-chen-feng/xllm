...
torch::Tensor WhisperFeatureExtractor::torch_extract_fbank_features_(
        ...
        if (dither_ != 0.0) {
            auto noise = torch::randn(waveform_tensor.sizes(), 
                                    waveform_tensor.options());
            waveform_tensor = waveform_tensor + dither_ * noise;
        }
        
        auto stft = torch::stft(waveform_tensor, n_fft_, hop_length_, n_fft_, 
                               window, true, "reflect", false, c10::nullopt,  true);
...
void WhisperFeatureExtractor::zero_mean_unit_var_norm_(
   ...
   const torch::Tensor& input_values =
        batched_feature.get<std::vector<torch::Tensor>>("input_features").value()[0];
   
  
   const auto& attention_mask_vector = batched_feature.get<std::vector<torch::Tensor>>("attention_mask");
   torch::Tensor attention_mask;
   if (attention_mask_vector.has_value() && !attention_mask_vector.value().empty())
...
Args:
    raw_speech: vector<torch::Tensor>
        raw_speech is expected to be a batch of tensor,  vector<torch::Tensor> (tensor, n), 
        with tensor shape [feature_len]
    BatchFeature output
        main_feature: expected to be a  vector<torch::Tensor> (tensor, 1),
        tensor with shape [padded_feature_len, n],
        mask: optional, expected to be a  vector<torch::Tensor> (tensor, 1),
        tensor with shape [padded_feature_len, n],
        num_frames: optional, expected to be a  vector<torch::Tensor> (tensor, 1),
        tensor with shape [n]
*/
bool WhisperFeatureExtractor::process(
      const MMInput& mm_inputs, 
      MMData& mm_datas 
      ) {
        //std::vector<torch::Tensor> raw_speech = mm_inputs.get_decode_data(MMType::AUDIO);
        auto state_dict = StateDictFromSafeTensor::load("/export/home/shanchenfeng/xllm_build/xllm_qwen_embed/qwen_omni_code/raw_speech.pt");
        auto input_featss = torch::ones({46421});
        bool is_conv_out_weight_loaded_ = false;
        weight::load_weight(*state_dict, "raw_speech", input_featss, is_conv_out_weight_loaded_);
        LOG(INFO) << is_conv_out_weight_loaded_;
        std::vector<torch::Tensor> raw_speech = {input_featss}; 
        ...
        for (const auto& speech : raw_speech) {
            if (speech.dim() > 1 ) {
                CHECK(false)
                      << "Only mono-channel audio is supported";
            }
            
            torch::Tensor processed_speech;
            if (speech.dim() == 1) {
                processed_speech = speech.unsqueeze(0).t();  // tensor shape [feature_len, 1]
            } else {
        ...
        if (do_normalize_) {
            zero_mean_unit_var_norm_(
                batched_feature, 
                padding_value_.value()
            );
        }
        torch::save(batched_feature.get<std::vector<torch::Tensor>>("input_features").value()[0], "feature_after.pt");
        torch::Tensor input_features;
        if (auto res = batched_feature.get<std::vector<torch::Tensor>>("input_features"))
             input_features = res.value()[0];
         
        input_features.print(); 
        input_features = input_features.permute({2, 0, 1});

        auto input_features_extracted = torch_extract_fbank_features_(input_features[0], options_);
        std::vector<torch::Tensor> input_features_extracted_vec{input_features_extracted};
        batched_feature.update("input_features", input_features_extracted_vec);
        
        if (return_attention_mask_) {
            torch::Tensor attention_mask;
            if (auto res = batched_feature.get<std::vector<torch::Tensor>>("attention_mask"))
                attention_mask = res.value()[0];
            
            auto rescaled_attention_mask = attention_mask.index(
                {torch::indexing::Slice(), torch::indexing::Slice(0, torch::indexing::None, hop_length_)}
            );
        ...
        if (return_token_timestamps_){
           for (auto& speech: raw_speech) {
               num_frames.push_back(torch::tensor(static_cast<int64_t>(speech.size(0)) / hop_length_, torch::kInt32));
           }
           batched_feature.add("num_frames", num_frames);
        }

        torch::save(batched_feature.get<std::vector<torch::Tensor>>("input_features").value()[0], "feature.pt");
        torch::save(batched_feature.get<torch::Tensor>("attention_mask").value()[0], "mask.pt");
        
        auto& item = mm_datas.add(MMType::AUDIO);
        item.set_data(batched_feature.data_);
