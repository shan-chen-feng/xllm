...
Args:
    ...
    masks: optional<vector<torch::Tensor>>
         should with same shape as input_features
*/
...
Args:
    ...
    BatchFeature output
        output expected to be a  vector<torch::Tensor> (tensor, 1),
        tensor with shape [pad_feature_len, n]
*/
BatchFeature  WhisperFeatureExtractor::process(
        const std::vector<torch::Tensor>& raw_speech,
        bool truncation,
        std::optional<int64_t> pad_to_multiple_of,
        std::optional<bool> return_tensors,
        ...
        bool return_token_timestamps
    ) {
        // raw_speech [n * tensor([length])] or [n * tensor([1, length])] 
        if (sampling_rate != -1 && sampling_rate != sampling_rate_) {
            CHECK(false)
                << "The model corresponding to this feature extractor was trained using a "
                "sampling rate of " + std::to_string(sampling_rate_) + 
                ". Please make sure that the provided `raw_speech` input "
                "was sampled with " + std::to_string(sampling_rate_) + 
                " and not " + std::to_string(sampling_rate.value()) + ".";
        ...
        bool do_normalize_ = do_normalize.has_value() && do_normalize.value();
        bool return_attention_mask_ = return_attention_mask.has_value() && return_attention_mask.value();
        
        pad(
            batched_feature,
            padding,
            max_length.has_value() ? max_length.value() : n_samples_,
            truncation,
            pad_to_multiple_of,
        ...
        const std::vector<torch::Tensor>& input_values =
             batched_feature.get<std::vector<torch::Tensor>>("input_features").value();
        auto input_features = input_values[0].permute({2, 0, 1});
        ...
        batched_feature.update("input_features", std::move(input_features_extracted_vec));
        if (return_attention_mask_) {
            const auto& attention_mask = batched_feature.get<std::vector<torch::Tensor>>("attention_mask").value()[0];
        ...
        if (return_token_timestamps){
           for (auto& speech: raw_speech) {
               num_frames.push_back(static_cast<int64_t>(speech.size(0)) / hop_length_);
           }
