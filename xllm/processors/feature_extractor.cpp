    ...
    void FeatureExtractor::pad(
        ...
        for (size_t i = 0; i < batch_size; i++) {
            ...
            for (const auto& [key, value] : data) {
                LOG(INFO) << key;
                std::get<std::vector<torch::Tensor>>(value)[i].print();
        ...
        if (padding == PaddingStrategy::LONGEST) {
            ...
            for (const auto& input_slice : truncated_inputs) {
                ...
                max_length = std::max(max_length.value(), static_cast<int64_t>(input.size(0)));
            }
        ...
        for (size_t i = 0; i < batch_size; i++) {
            ...
            for (const auto& [key, value] : data) {
                ...
                LOG(INFO) << key;
                std::get<torch::Tensor>(value).print();
        ...
        for (const auto& [key, value] : batch_outputs) {
           ...
           processed_features.get<std::vector<torch::Tensor>>("input_features").value()[0].print();
        }
        processed_features.get<std::vector<torch::Tensor>>("input_features").value()[0].print();
        LOG(INFO) << "end stack";
