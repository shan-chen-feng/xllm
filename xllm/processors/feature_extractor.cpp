...
namespace xllm {
    ...
    Args:
       ...
       input_features: vector<torch::Tensor>
           input: input_features is expected to be a vector<torch::Tensor> (tensor, n), 
           with tensor shape [feature_len, 1] 
            
           output: updata input_features to a  vector<torch::Tensor> (tensor, 1), 
           with tensor shape [n, pad_feature_len, 1]
    */    
    void FeatureExtractor::pad(
        ...
        LOG(INFO) << "start pad process";
        CHECK(!model_input_names_.empty() && model_input_names_.size() > 0)
              << "model_input_names should be a vector of string that contains the input name, "
              << "but got an empty vector";

        std::string main_input_name = model_input_names_[0];
        const auto& input =  processed_features.get<std::vector<torch::Tensor>>(main_input_name);

        CHECK(input.has_value())
              << "You should supply an instance of BatchFeatures to this method that contains "
              << main_input_name;
        ...
        if (padding == PaddingStrategy::LONGEST) {
            int64_t max_length_ = max_length.has_value() ? max_length.value() : 0;
            for (const auto& input_slice : truncated_inputs) {
                const auto& input = input_slice.get<torch::Tensor>(main_input_name).value();
                max_length_ = std::max(max_length_, static_cast<int64_t>(input.size(0)));
            }
        ...
        for (const auto& [key, value] : batch_outputs) {
           ...
           processed_features.update(key, std::move(stack_vec));
        }
        
    }
    
    void FeatureExtractor::pad_(
        BatchFeature& processed_features,
        ...
        LOG(INFO) << "start pad_ process";
        std::string main_input_name = model_input_names_[0];
        const auto& required_input = processed_features.get<torch::Tensor>(main_input_name).value();
        
        if (padding_strategy == PaddingStrategy::LONGEST)
            max_length = required_input.size(0);       

        if (max_length.has_value() && pad_to_multiple_of.has_value() && (max_length.value() % pad_to_multiple_of.value() != 0)) {
            max_length = ((max_length.value() / pad_to_multiple_of.value()) + 1) * pad_to_multiple_of.value();
        }
        LOG(INFO) << "max_length is " << max_length.value();
        
        bool needs_padding = padding_strategy != PaddingStrategy::DO_NOT_PAD && 
                           static_cast<int64_t>(required_input.size(0)) < max_length.value();
        
        if (return_attention_mask.has_value() && return_attention_mask.value() && !processed_features.has("attention_mask")) {
            auto attention_mask = torch::ones({required_input.size(0)}, torch::kInt32);
            processed_features.add("attention_mask", std::move(attention_mask));
        }
        ...
        if (needs_padding) {
            ...
            const auto& attention_mask = processed_features.get<torch::Tensor>("attention_mask").value();
            
            if (padding_side_ == "right") {
                if (return_attention_mask.has_value() && return_attention_mask.value()) {
                    auto padded_mask = torch::constant_pad_nd(attention_mask, {0, difference}, 0);
                    processed_features.update("attention_mask", std::move(padded_mask));
                }
                
                std::vector<int64_t> pad_shape;
                if (feature_size_ > 1) {
                    pad_shape = {0, difference, 0, 0};  
                } else {
                    pad_shape = {0, difference};
                }
                
                auto padded_input = torch::constant_pad_nd(required_input, pad_shape, padding_value_.value());
                processed_features.update(main_input_name, std::move(padded_input));
                
            } else if (padding_side_ == "left") {
                if (return_attention_mask) {
                    auto padded_mask = torch::constant_pad_nd(attention_mask, {difference, 0}, 0);
                    processed_features.update("attention_mask", std::move(padded_mask));
                }
                
                std::vector<int64_t> pad_shape;
                if (feature_size_ > 1) {
                    pad_shape = {difference, 0, 0, 0};
                } else {
               ...
               CHECK(false)  
                   <<  "Invalid padding side: " <<  padding_side_;
            }
        }
        
    }
    
    void FeatureExtractor::truncate_(
        BatchFeature& processed_features,
        std::optional<int64_t> max_length,
        std::optional<int64_t> pad_to_multiple_of,
        ...
        if (needs_truncation) {
            ...
            processed_features.update(main_input_name, std::move(truncated_input));
            
            if (processed_features.has("attention_mask")) {
                const auto& attention_mask = processed_features.get<torch::Tensor>("attention_mask").value();
                auto truncated_mask = attention_mask.slice(0, 0, max_length.value());
                processed_features.update("attention_mask", std::move(truncated_mask));
            }
        }
    }
   ...
   void FeatureExtractor::check_padding_strategy_(
        ...
        if (!max_length.has_value() && padding == PaddingStrategy::MAX_LENGTH) {
            CHECK(false) 
              <<  "When using max_length padding, max_length must be defined";
        }
        
        if (padding != PaddingStrategy::DO_NOT_PAD && !padding_value_.has_value()) {
            CHECK(false)
              <<  "Padding value is not set. Please set a padding_value.";
        }
